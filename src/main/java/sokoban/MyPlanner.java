...
public class MyPlanner extends AbstractPlanner {
    ...
    public Plan solve(final Problem problem) {
        ...
        if (plan != null) {
            ...
            // Parcourir chaque action et récupérer la première lettre
            actions.forEach(action -> {
                String actionName = action.getName().trim(); // Récupérer le nom de l'action
                // LOGGER.info("Action trouvée : " + actionName); // Affichage de chaque action
                // pour vérification

                // Extraire la première lettre de l'action
                if (!actionName.isEmpty()) {
                    firstLetters.append(actionName.charAt(0)); // Ajouter la première lettre à la chaîne
                }
            });

            //ecrire la solution dans le fichier solution.txt
            try (BufferedWriter writer = new BufferedWriter(new FileWriter("src/main/java/sokoban/solution.txt"))) {
                writer.write(firstLetters.toString().toUpperCase());
            } catch (IOException e) {
                e.printStackTrace();
            }

            // Affichage des premières lettres des actions
            LOGGER.info("Les premieres lettres des actions : " + firstLetters.toString());
        } else {
